...
async function recharge(page, browser) {
    ...
    try {
        ...
        await page.getByRole('textbox', { name: 'Please enter your search' }).fill(accountName);
        await page.getByRole('button', { name: ' search' }).click();

        // 3. Wait for the result row, or abort if none
        const resultRow = page.locator('tbody tr').filter({ hasText: accountName }).first();
        try {
        ...
        await page.locator('li.el-dropdown-menu__item').filter({ hasText: 'Recharge' }).click();
        await page.getByRole('dialog', { name: 'Please confirm your recharge' }).getByRole('spinbutton').fill(rechargeAmount);
        await page.getByRole('dialog', { name: 'Please confirm your recharge' }).locator('textarea').fill(remarks);
