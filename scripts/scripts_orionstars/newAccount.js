...
const username = process.argv[2] || 'Test__os';
...
async function loginAndSaveState() {
  ...
  try {
    ...
    await page.goto('https://orionstars.vip:8781/default.aspx');
...
async function createNewAccount(page, browser) {
    const newAccountName = 'testing03';
    ...
    await page.locator('iframe[name="frm_main_content"]').contentFrame().getByRole('textbox', { name: 'ID or Account' }).fill(newAccountName);
    await page.locator('iframe[name="frm_main_content"]').contentFrame().getByRole('link', { name: 'Search' }).click();
    ...
    await page.locator('#Container iframe').contentFrame().getByRole('link', { name: 'Create Player' }).click();
    // await page.getByText('The account number already').click();

    // Wait for the pop‑up div on the main page
    await page.waitForSelector('#mb_msg', { visible: true, timeout: 5000 });
    const popupText = (await page.locator('#mb_msg').textContent()).trim();
