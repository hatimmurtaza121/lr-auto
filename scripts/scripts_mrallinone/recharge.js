...
async function loginAndSaveState() {
  ...
  try {
    ...
    try {
      ...
      console.log('Login successful! Now proceeding to recharge...');
      // Run the createNewAccount function
      await recharge(page, browser);
...
async function recharge(page, browser) {

    //---------------------------------------------------------------------------------------------
    try {
        const accountName = "testing01";
        const rechargeAmount = "1";
        ...
        await page.locator('iframe').nth(1).contentFrame().locator('iframe[name="layui-layer-iframe2"]').contentFrame().getByRole('textbox', { name: 'Please enter Recharge Balance' }).fill(rechargeAmount);
