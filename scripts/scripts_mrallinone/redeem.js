...
async function redeem(page, browser) {
  ...
  try {
      ...
      await searchFrame.getByRole('textbox', { name: 'Please enter Username' }).fill(accountName);
      ...
      const resultRow = searchFrame.locator('tbody > tr').first();
      const balanceText = (
        await resultRow.locator('td').nth(5).textContent()
      ).trim();
      // parse it into a number (strip out any commas/currency symbols)
      ...
      await searchFrame
        ...
        .first()             // in case there are other buttons on the page
        .click();
      
      await page.locator('iframe').nth(1).contentFrame().locator('iframe[name="layui-layer-iframe2"]').contentFrame().getByRole('textbox', { name: 'Please enter Withdraw Balance' }).fill(redeemAmount);
      await page.locator('iframe').nth(1).contentFrame().locator('iframe[name="layui-layer-iframe2"]').contentFrame().getByRole('textbox', { name: 'Please enter Remarks' }).fill(remarks);
      await page.locator('iframe').nth(1).contentFrame().locator('iframe[name="layui-layer-iframe2"]').contentFrame().getByRole('button', { name: 'Submit' }).click();
    ...
    console.error('Error during redeem amount:', error);
  }
