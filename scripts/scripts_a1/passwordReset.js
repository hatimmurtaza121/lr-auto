...
async function resetAccountPassword(page, browser) {
  ...
  try {
    ...
    // 2. Wait for the layer iframe to be attached, then grab its Frame
    const popupFrame = await searchFrame
    .locator('iframe[name="layui-layer-iframe1"]')
    .waitFor({ timeout: 5000 })
    .then(() => searchFrame.locator('iframe[name="layui-layer-iframe1"]').contentFrame());

    // 3. Fill in the new passwords
    await popupFrame.getByRole('textbox', { name: 'Please enter Login password' }).fill(newPassword);
    await popupFrame.getByRole('textbox', { name: 'Please enter Confirm password' }).fill(newPassword);

    // 4. Submit
    await popupFrame.getByRole('button', { name: 'Submit' }).click();
