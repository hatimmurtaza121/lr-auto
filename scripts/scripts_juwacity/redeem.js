...
async function redeem(page, browser) {
    ...
    try {
        ...
        await page.locator('li.el-dropdown-menu__item').filter({ hasText: 'Redeem' }).click();
        await page.getByRole('dialog', { name: 'Please confirm your redeem &' }).getByRole('spinbutton').fill(redeemAmount);
        await page.getByRole('dialog', { name: 'Please confirm your redeem &' }).locator('textarea').fill(remarks);
