...
async function resetAccountPassword(page, browser) {
  ...
  try {
    ...
    await page.locator('li.el-dropdown-menu__item').filter({ hasText: 'Reset Password' }).click();

    // 5. Fill in the new password in the Reset Password dialog
    await page.locator('div').filter({ hasText: /^New password$/ })
      .getByRole('textbox')
      .fill(newPassword);

    await page.locator('div').filter({ hasText: /^Confirm password$/ })
      .getByRole('textbox')
      .fill(newPassword);
